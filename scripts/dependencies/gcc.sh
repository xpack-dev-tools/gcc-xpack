  ...
  then
    ...
    (
      ...
      (
        ...
        then
          # If the  was compiled with shared libs, copy
          # libwinpthread.dll here, since it'll be referenced by
          # several executables.
          # For just in case, currently the builds are static.
          if [ -f "${XBB_EXECUTABLES_INSTALL_FOLDER_PATH}/${XBB_TARGET_TRIPLET}/bin/libwinpthread-1.dll" ]
          then
            run_verbose install -c -m 755 "${XBB_EXECUTABLES_INSTALL_FOLDER_PATH}/${XBB_TARGET_TRIPLET}/bin/libwinpthread-1.dll" \
