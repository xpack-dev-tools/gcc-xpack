---

title: xPack GCC v11.5.0-2 released
seo_title: Version 11.5.0-2 released
description: Version 11.5.0-2 is a maintenance release.
keywords:
  - xpack
  - gcc
  - release

date: 2025-02-17 13:12:54 +0200

authors: ilg-ul

# To be listed in the Releases page.
tags:
  - releases

# ----- Custom properties -----------------------------------------------------

gcc_version: "11.5.0"
binutils_version: "2.42"
mingw_version: "12.0.0"
gdb_version: "14.2"

version: "11.5.0-2"
npm_subversion: "1"

download_url: https://github.com/xpack-dev-tools/gcc-xpack/releases/tag/v11.5.0-2/

---

import {PageMetadata} from '@docusaurus/theme-common';
import Image from '@theme/IdealImage';
import CodeBlock from '@theme/CodeBlock';
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

import Prerequisites from './_common/_prerequisites-glib-2.28.mdx';
import DeprecationNotices from './_common/_deprecation-notices-glib-2.28.mdx';
import DownloadAnalytics from './_common/_download-analytics.mdx';

Version **11.5.0-2.1** is a maintenance release; iit fixes the
unintended dependency on `libiconv.so.2`.

<!-- truncate -->

<PageMetadata title={frontMatter.seo_title} />

The [xPack GCC](https://xpack-dev-tools.github.io/gcc-xpack/)
is a standalone cross-platform binary distribution of
[GCC](https://gcc.gnu.org).

There are separate binaries for **Windows** (x64),
**macOS** (x64 and arm64)
and **GNU/Linux** (x64, arm64 and arm).

:::note Raspberry Pi

The main targets for the GNU/Linux Arm
binaries are the **Raspberry Pi** class devices (aarch64;
armv7l and armv6 are not supported).

:::

## Download

The binary files can be downloaded automatically with **xpm** or manually
from <a href={frontMatter.download_url}>GitHub Releases</a>.

<Prerequisites/>

## Install

The easiest way to install this specific version, is by using **xpm**:

<CodeBlock language="console"> {
`xpm install @xpack-dev-tools/gcc@${frontMatter.version}.${frontMatter.npm_subversion} -verbose
`} </CodeBlock>

Comprehensive instructions for installing **xPack GCC** on different platforms
can be found in the [Install Guide](/docs/install/).

## Compliance

The xPack GCC uses the official sources,
with no functional changes:

- GCC {frontMatter.gcc_version}
- binutils {frontMatter.binutils_version}
- mingw-w64 {frontMatter.mingw_version}
- gdb {frontMatter.gdb_version}

## Supported languages

The supported languages are:

- C
- C++
- Obj-C
- Obj-C++
- Fortran

Note: Obj-C/C++ support is minimalistic.

Starting with 12.2.0-2, support for 32-bit multilib was added on Intel
Linux; it can be enabled via the `-m32` compile option.

## Changes

Compared to the upstream, there are no functional changes.

## Bug fixes

- [[#9](https://github.com/xpack-dev-tools/gcc-xpack/issues/9)]:
due to an issue with the build scripts, in the previous releases
(14.2.0-1, 13.3.0-1, 12.4.0-1, 11.5.0-1) `libstdc++.so.6` has an
unintended dependency on `libiconv.so.2`, which may be absent in
some configurations; fixed.

## Enhancements

- none

## Known problems

Please see the separate
[tests results page](/docs/tests/11.5.0-2/).

## Documentation

The original documentation is available from
the project web site:

- https://gcc.gnu.org/onlinedocs/

## Build

The binaries for all supported platforms
(Windows, macOS and GNU/Linux) were built using the
[xPack Build Box (XBB)](https://xpack.github.io/xbb/), a set
of build environments based on slightly older distributions, that should be
compatible with most recent systems.

For the prerequisites and more details on the build procedure, please see the
[Maintainer Info](/docs/maintainer/) page.

## CI tests

Before publishing, a set of simple tests were performed on an exhaustive
set of platforms. The results are available from:

- [GitHub Actions](https://github.com/xpack-dev-tools/gcc-xpack/actions/)
- [Travis CI](https://app.travis-ci.com/github/xpack-dev-tools/gcc-xpack/builds/)

## Tests

The binaries were tested on a variety of platforms,
but mainly to check the integrity of the
build, not the compiler functionality.

## Checksums

The SHA-256 hashes for the files are:

```txt
68c8717f2595ef5cf5d1fc2ea352321bcc74fc4e4c682bebba6deb627ece97b5
xpack-gcc-11.5.0-2-linux-arm64.tar.gz

853067193ac7f22ada75038e13a40bad01874625248edaaf11c32135eba7da8b
xpack-gcc-11.5.0-2-linux-x64.tar.gz

5a94c9bf01c547fca24a1bd2f99d93e9970abf808997e154cba67d3e754322a0
xpack-gcc-11.5.0-2-win32-x64.zip

```

## Folders hierarchy

After install, the package creates a hierarchy of folders like the following
(only the first two depth levels are shown):

<Tabs groupId="operating-systems">

<TabItem value="windows" label="Windows" default>

<CodeBlock language="console"> {
`C:\> tree /f %USERPROFILE%\\AppData\\Roaming\\xPacks\\@xpack-dev-tools\\gcc\\${frontMatter.version}.${frontMatter.npm_subversion}\\.content
Folder PATH listing
Volume serial number is B02D-925C
├── README.md
├── bin
│   ├── addr2line.exe
│   ├── ar.exe
│   ├── as.exe
│   ├── c++.exe
│   ├── c++filt.exe
│   ├── cpp.exe
│   ├── dlltool.exe
│   ├── dllwrap.exe
│   ├── elfedit.exe
│   ├── g++.exe
│   ├── gcc-ar.exe
│   ├── gcc-nm.exe
│   ├── gcc-ranlib.exe
│   ├── gcc.exe
│   ├── gcov-dump.exe
│   ├── gcov-tool.exe
│   ├── gcov.exe
│   ├── gdb-add-index
│   ├── gdb.exe
│   ├── gendef.exe
│   ├── gfortran.exe
│   ├── gprof.exe
│   ├── ld.bfd.exe
│   ├── ld.exe
│   ├── libatomic-1.dll
│   ├── libgcc_s_seh-1.dll
│   ├── libgfortran-5.dll
│   ├── libgmp-10.dll
│   ├── libgomp-1.dll
│   ├── libmpc-3.dll
│   ├── libmpfr-6.dll
│   ├── libquadmath-0.dll
│   ├── libssp-0.dll
│   ├── libstdc++-6.dll
│   ├── libzstd.dll
│   ├── lto-dump.exe
│   ├── nm.exe
│   ├── objcopy.exe
│   ├── objdump.exe
│   ├── ranlib.exe
│   ├── readelf.exe
│   ├── size.exe
│   ├── strings.exe
│   ├── strip.exe
│   ├── widl.exe
│   ├── windmc.exe
│   └── windres.exe
├── distro-info
│   └── licenses
├── include
│   ├── c++
│   └── gdb
├── lib
│   ├── gcc
│   ├── libatomic-1.dll
│   ├── libatomic.a
│   ├── libatomic.dll.a
│   ├── libatomic.la
│   ├── libgcc_s.a
│   ├── libgcc_s_seh-1.dll
│   ├── libgfortran-5.dll
│   ├── libgfortran.a
│   ├── libgfortran.dll.a
│   ├── libgfortran.la
│   ├── libgfortran.spec
│   ├── libgomp-1.dll
│   ├── libgomp.a
│   ├── libgomp.dll.a
│   ├── libgomp.la
│   ├── libgomp.spec
│   ├── libobjc-4.dll
│   ├── libquadmath-0.dll
│   ├── libquadmath.a
│   ├── libquadmath.dll.a
│   ├── libquadmath.la
│   ├── libssp-0.dll
│   ├── libssp.a
│   ├── libssp.dll.a
│   ├── libssp.la
│   ├── libssp_nonshared.a
│   ├── libssp_nonshared.la
│   ├── libstdc++-6.dll
│   ├── libstdc++.a
│   ├── libstdc++.dll.a
│   ├── libstdc++.dll.a-gdb.py
│   ├── libstdc++.la
│   ├── libstdc++exp.a
│   ├── libstdc++exp.la
│   ├── libstdc++fs.a
│   ├── libstdc++fs.la
│   ├── libsupc++.a
│   └── libsupc++.la
├── libexec
│   └── gcc
├── share
│   ├── gcc-11.5.0
│   └── gdb
└── x86_64-w64-mingw32
    ├── bin
    ├── include
    └── lib

17 directories, 84 files
`} </CodeBlock>

</TabItem>

<TabItem value="linux" label="GNU/Linux">

<CodeBlock language="console"> {
`$ tree -L 2 ~/.local/xPacks/\@xpack-dev-tools/gcc/${frontMatter.version}.${frontMatter.npm_subversion}/.content/
/home/ilg/.local/xPacks/\@xpack-dev-tools/gcc/${frontMatter.version}.${frontMatter.npm_subversion}/.content/
├── README.md
├── bin
│   ├── addr2line
│   ├── ar
│   ├── as
│   ├── c++
│   ├── c++filt
│   ├── cpp
│   ├── dwp
│   ├── elfedit
│   ├── g++
│   ├── gcc
│   ├── gcc-ar
│   ├── gcc-nm
│   ├── gcc-ranlib
│   ├── gcore
│   ├── gcov
│   ├── gcov-dump
│   ├── gcov-tool
│   ├── gdb
│   ├── gdb-add-index
│   ├── gfortran
│   ├── gp-archive
│   ├── gp-collect-app
│   ├── gp-display-html
│   ├── gp-display-src
│   ├── gp-display-text
│   ├── gprof
│   ├── gprofng
│   ├── ld
│   ├── ld.bfd
│   ├── ld.gold
│   ├── lto-dump
│   ├── nm
│   ├── objcopy
│   ├── objdump
│   ├── ranlib
│   ├── readelf
│   ├── size
│   ├── strings
│   └── strip
├── distro-info
│   └── licenses
├── etc
│   └── gprofng.rc
├── include
│   ├── c++
│   └── gdb
├── lib
│   └── gcc
├── lib32
│   ├── libasan.a
│   ├── libasan.la
│   ├── libasan.so -> libasan.so.8.0.0
│   ├── libasan.so.8 -> libasan.so.8.0.0
│   ├── libasan.so.8.0.0
│   ├── libasan_preinit.o
│   ├── libatomic.a
│   ├── libatomic.la
│   ├── libatomic.so -> libatomic.so.1.2.0
│   ├── libatomic.so.1 -> libatomic.so.1.2.0
│   ├── libatomic.so.1.2.0
│   ├── libgcc_s.so
│   ├── libgcc_s.so.1
│   ├── libgfortran.a
│   ├── libgfortran.la
│   ├── libgfortran.so -> libgfortran.so.5.0.0
│   ├── libgfortran.so.5 -> libgfortran.so.5.0.0
│   ├── libgfortran.so.5.0.0
│   ├── libgfortran.spec
│   ├── libgomp.a
│   ├── libgomp.la
│   ├── libgomp.so -> libgomp.so.1.0.0
│   ├── libgomp.so.1 -> libgomp.so.1.0.0
│   ├── libgomp.so.1.0.0
│   ├── libgomp.spec
│   ├── libitm.a
│   ├── libitm.la
│   ├── libitm.so -> libitm.so.1.0.0
│   ├── libitm.so.1 -> libitm.so.1.0.0
│   ├── libitm.so.1.0.0
│   ├── libitm.spec
│   ├── libquadmath.a
│   ├── libquadmath.la
│   ├── libquadmath.so -> libquadmath.so.0.0.0
│   ├── libquadmath.so.0 -> libquadmath.so.0.0.0
│   ├── libquadmath.so.0.0.0
│   ├── libsanitizer.spec
│   ├── libssp.a
│   ├── libssp.la
│   ├── libssp.so -> libssp.so.0.0.0
│   ├── libssp.so.0 -> libssp.so.0.0.0
│   ├── libssp.so.0.0.0
│   ├── libssp_nonshared.a
│   ├── libssp_nonshared.la
│   ├── libstdc++.a
│   ├── libstdc++.la
│   ├── libstdc++.so -> libstdc++.so.6.0.29
│   ├── libstdc++.so.6 -> libstdc++.so.6.0.29
│   ├── libstdc++.so.6.0.29
│   ├── libstdc++.so.6.0.29-gdb.py
│   ├── libstdc++fs.a
│   ├── libstdc++fs.la
│   ├── libsupc++.a
│   ├── libsupc++.la
│   ├── libubsan.a
│   ├── libubsan.la
│   ├── libubsan.so -> libubsan.so.1.0.0
│   ├── libubsan.so.1 -> libubsan.so.1.0.0
│   └── libubsan.so.1.0.0
├── lib64
│   ├── libasan.a
│   ├── libasan.la
│   ├── libasan.so -> libasan.so.6.0.0
│   ├── libasan.so.8 -> libasan.so.6.0.0
│   ├── libasan.so.6.0.0
│   ├── libasan_preinit.o
│   ├── libatomic.a
│   ├── libatomic.la
│   ├── libatomic.so -> libatomic.so.1.2.0
│   ├── libatomic.so.1 -> libatomic.so.1.2.0
│   ├── libatomic.so.1.2.0
│   ├── libcc1.a
│   ├── libcc1.la
│   ├── libcc1.so -> libcc1.so.0.0.0
│   ├── libcc1.so.0 -> libcc1.so.0.0.0
│   ├── libcc1.so.0.0.0
│   ├── libgcc_s.so
│   ├── libgcc_s.so.1
│   ├── libgfortran.a
│   ├── libgfortran.la
│   ├── libgfortran.so -> libgfortran.so.5.0.0
│   ├── libgfortran.so.5 -> libgfortran.so.5.0.0
│   ├── libgfortran.so.5.0.0
│   ├── libgfortran.spec
│   ├── libgomp.a
│   ├── libgomp.la
│   ├── libgomp.so -> libgomp.so.1.0.0
│   ├── libgomp.so.1 -> libgomp.so.1.0.0
│   ├── libgomp.so.1.0.0
│   ├── libgomp.spec
│   ├── libhwasan.a
│   ├── libhwasan.la
│   ├── libhwasan.so -> libhwasan.so.0.0.0
│   ├── libhwasan.so.0 -> libhwasan.so.0.0.0
│   ├── libhwasan.so.0.0.0
│   ├── libhwasan_preinit.o
│   ├── libitm.a
│   ├── libitm.la
│   ├── libitm.so -> libitm.so.1.0.0
│   ├── libitm.so.1 -> libitm.so.1.0.0
│   ├── libitm.so.1.0.0
│   ├── libitm.spec
│   ├── liblsan.a
│   ├── liblsan.la
│   ├── liblsan.so -> liblsan.so.0.0.0
│   ├── liblsan.so.0 -> liblsan.so.0.0.0
│   ├── liblsan.so.0.0.0
│   ├── liblsan_preinit.o
│   ├── libquadmath.a
│   ├── libquadmath.la
│   ├── libquadmath.so -> libquadmath.so.0.0.0
│   ├── libquadmath.so.0 -> libquadmath.so.0.0.0
│   ├── libquadmath.so.0.0.0
│   ├── libsanitizer.spec
│   ├── libssp.a
│   ├── libssp.la
│   ├── libssp.so -> libssp.so.0.0.0
│   ├── libssp.so.0 -> libssp.so.0.0.0
│   ├── libssp.so.0.0.0
│   ├── libssp_nonshared.a
│   ├── libssp_nonshared.la
│   ├── libstdc++.a
│   ├── libstdc++.la
│   ├── libstdc++.so -> libstdc++.so.6.0.29
│   ├── libstdc++.so.6 -> libstdc++.so.6.0.29
│   ├── libstdc++.so.6.0.29
│   ├── libstdc++.so.6.0.29-gdb.py
│   ├── libstdc++fs.a
│   ├── libstdc++fs.la
│   ├── libsupc++.a
│   ├── libsupc++.la
│   ├── libtsan.a
│   ├── libtsan.la
│   ├── libtsan.so -> libtsan.so.0.0.0
│   ├── libtsan.so.0 -> libtsan.so.0.0.0
│   ├── libtsan.so.0.0.0
│   ├── libtsan_preinit.o
│   ├── libubsan.a
│   ├── libubsan.la
│   ├── libubsan.so -> libubsan.so.1.0.0
│   ├── libubsan.so.1 -> libubsan.so.1.0.0
│   └── libubsan.so.1.0.0
├── libexec
│   ├── gcc
│   ├── libbfd-2.42.so
│   ├── libctf-nobfd.so.0 -> libctf-nobfd.so.0.0.0
│   ├── libctf-nobfd.so.0.0.0
│   ├── libctf.so.0 -> libctf.so.0.0.0
│   ├── libctf.so.0.0.0
│   ├── libexpat.so.1 -> libexpat.so.1.9.2
│   ├── libexpat.so.1.9.2
│   ├── libfl.so.2 -> libfl.so.2.0.0
│   ├── libfl.so.2.0.0
│   ├── libgcc_s.so.1
│   ├── libgmp.so.10 -> libgmp.so.10.5.0
│   ├── libgmp.so.10.5.0
│   ├── libgprofng.so.0 -> libgprofng.so.0.0.0
│   ├── libgprofng.so.0.0.0
│   ├── libiconv.so.2 -> libiconv.so.2.6.1
│   ├── libiconv.so.2.6.1
│   ├── libisl.so.23 -> libisl.so.23.3.0
│   ├── libisl.so.23.3.0
│   ├── liblzma.so.5 -> liblzma.so.5.4.6
│   ├── liblzma.so.5.4.6
│   ├── libmpc.so.3 -> libmpc.so.3.3.1
│   ├── libmpc.so.3.3.1
│   ├── libmpfr.so.6 -> libmpfr.so.6.2.1
│   ├── libmpfr.so.6.2.1
│   ├── libncursesw.so.6 -> libncursesw.so.6.4
│   ├── libncursesw.so.6.4
│   ├── libopcodes-2.42.so
│   ├── libsframe.so.1 -> libsframe.so.1.0.0
│   ├── libsframe.so.1.0.0
│   ├── libstdc++.so.6 -> libstdc++.so.6.0.33
│   ├── libstdc++.so.6.0.33
│   ├── libz.so.1 -> libz.so.1.3.1
│   ├── libz.so.1.3.1
│   ├── libzstd.so.1 -> libzstd.so.1.5.6
│   └── libzstd.so.1.5.6
├── share
│   ├── gcc-11.5.0
│   └── gdb
└── x86_64-pc-linux-gnu
    ├── bin
    └── lib

19 directories, 211 files
`} </CodeBlock>

</TabItem>

</Tabs>

<DeprecationNotices/>

<DownloadAnalytics version={frontMatter.version}/>
