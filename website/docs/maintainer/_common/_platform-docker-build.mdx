...
```sh
xpm run clear-all-project-builds -C ~/Work/xpack-dev-tools/gcc-xpack.git/build-assets
```
...
* <code>xpack-gcc-{customField('upstreamVersion')}-{customField('xpackSubversion')}-{props.platform}.{props.platform === 'win32-x64' ? 'zip' : 'tar.gz'}</code>
* <code>xpack-gcc-{customField('upstreamVersion')}-{customField('xpackSubversion')}-{props.platform}.{props.platform === 'win32-x64' ? 'zip' : 'tar.gz'}.sha</code>
