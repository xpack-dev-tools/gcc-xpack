...
**@xpack-dev-tools/gcc** is an xpm package that packages platform-specific archives, built with the [xPack Build Box (XBB)](https://xpack.github.io/xpack-build-box/).
...
The builds currently execute on 2 dedicated machines (x64 GNU/Linux, arm64 GNU/Linux).
...
(such as **15.2.0-1.1**) with the following expression:
...
xpm run trigger-workflow-test-docker-linux-arm64 -C ~/Work/xpack-dev-tools/gcc-xpack.git/build-assets
```
...
npm --prefix ~/Work/xpack-dev-tools/gcc-xpack.git/build-assets install
xpm install -C ~/Work/xpack-dev-tools/gcc-xpack.git/build-assets
...
- In `CHANGELOG.md`, add the release date and a message
  such as _**\* v15.2.0-1 released**_
...
tagged such as **v15.2.0-1** (mind the dash in the middle!) and
named such as **xPack GCC v15.2.0-1** (mind the dash),
...
- **Commit** all changes with the message
  _**package.json: update URLs for 15.2.0-1.1 release**_
...
- Navigate to the [GitHub Releases](https://github.com/xpack-dev-tools/gcc-xpack/releases) page
...
npm --prefix ~/Work/xpack-dev-tools/gcc-xpack.git/build-assets install && \
xpm install -C ~/Work/xpack-dev-tools/gcc-xpack.git/build-assets && \
xpm run test-release -C ~/Work/xpack-dev-tools/gcc-xpack.git/build-assets
```
