...
named like **xPack GCC v{customField('xpackVersion')}** (mind the dash),
...
<CodeBlock language="sh"> {
`npm unpublish @xpack-dev-tools/gcc@${customField('xpackVersion')}.${customField('npmSubversion')}
`} </CodeBlock>
...
- paste the release name like _**xPack GCC v{customField('xpackVersion')} released**_
